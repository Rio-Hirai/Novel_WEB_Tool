...
begin
  ...
  db.transaction() {
    ...
    if pass.empty? # パスワードがないなら
      choices.push("<li><form method='post' name='form1' action=#{url}><input type='hidden' name='id' value=#{line[0]}>
<input type='hidden' name='name' value=#{line[1]}><a href='javascript:form1[#{cnt}].submit()'>
#{line[1]}</a>   作成者：#{line[2]}</form></li>")
    else # パスワードがあるなら
      choices.push("<li><form method='post' name='form1' action=#{url} onclick='return gate(\"#{line[4]}\")'>
...
begin
  ...
  <h1>#{name}</h1>
  <hr><h2><ul>
  <li><form method='post' name='form1' action=#{urlw}>
  <a href='javascript:form1[0].submit()'>用語</a>
  </form></li>
  <li><form method='post' name='form1' action=#{urlc}>
...
begin
  ...
  <h1>新しい世界をつくる</h1>
  <h2>
  <form action="http://cgi.u.tsukuba.ac.jp/~s1711466/local_only/wp/world_create2.rb" method="post">
...
begin
  ...
  <html><body>
  <p>新しい用語を追加しました</p>
  <form method='post' name='form1' action="http://cgi.u.tsukuba.ac.jp/~s1711466/local_only/wp/words.rb">
...
begin
  ...
  <h1>用語を編集する</h1>
  <h2>
  <form action="http://cgi.u.tsukuba.ac.jp/~s1711466/local_only/wp/words_edit2.rb" method="post" 
...
begin
  ...
  <html><body>
  <p>用語の情報を編集しました</p>
  <form method='post' name='form1' action="http://cgi.u.tsukuba.ac.jp/~s1711466/local_only/wp/words_page.rb">
  <input type='hidden' name='wid' value=#{wid}>
  <a href='javascript:form1.submit()'>戻る</a>
  </body></html>
  EOB
...
begin
  ...
  db.transaction() {
    ...
    # 用語ごとに個別ページのリンクを生成
    db.execute(sql) {|line|
    choices.push("<li><form method='post' name='form1' action=#{url}><input type='hidden' name='cid' value=#{line[0]}>
...
begin
  ...
  <form method='post' name='form1' action="http://cgi.u.tsukuba.ac.jp/~s1711466/local_only/wp/chara.rb">
  <a href='javascript:form1.submit()'>戻る</a>
  </body></html>
  EOB
  session.close
...
begin
  ...
  <h1>キャラクターを編集する</h1>
  <h2>
  <form action="http://cgi.u.tsukuba.ac.jp/~s1711466/local_only/wp/chara_edit2.rb" method="post" 
onSubmit="return gate('#{pass[0]}')">
  <p>作成者<input type="text" name="creator"></p>
  <p>キャラクター名<input type="text" name="chara"></p>
  <p>読み<input type="text" name="ruby"></p>
